# -*- coding: utf-8 -*-
"""
Created on Wed Aug 30 23:15:37 2023
...
# List of coins
coin_list =  [
    "ethereum", "eth_classic", "golem",
    "basic_attention_token", "power_ledger", "request",
    "substratum", "aeternity", "waltonchain",
    "civic", "digixdao", "iexec_rlc",
    "airdao", "district0x", "sirin_labs_token",
    "metal", "gifto", "delphy",
    "binance", "eos", "chainlink",
    "tron"
]
...
for coin in coin_list:
    ...
    data=pd.read_excel("C:/Users/Kevin Meng/OneDrive/Desktop/Dissertation/price_data.xlsx",sheet_name=coin)
    
    desired_start_date = data['Date'].min()
    ...
    def build_lstm_model(hp):
        model = Sequential()
        model.add(LSTM(hp.Int('units', min_value=32, max_value=128, step=32), activation='relu', input_shape=(trainX.shape[1], trainX.shape[2]), return_sequences=False))
        #model.add(LSTM(hp.Int('units', min_value=16, max_value=64, step=16)//2, activation='relu', return_sequences=False))
    ...
    average_mae_list.append((coin, average_mae))
    ...
    train_mae = mean_absolute_error(testY, y_pred_test)
    
    # Append the results to train_mae_scores
    train_mae_scores.append((coin, train_mae))
...
for coin, average_mae in average_mae_list:
    print(f"Coin: {coin}, Average MAE: {average_mae}")